<template>
  <div ref="myAppContainer" class="app-container">
    <div class="page-title">带货主播榜</div>
    <fliterArea :use-number="2" :title-name="'主播类型'" :is-history="false" :is-anchor-data="false" @getData="getData" />
    <listScreen ref="listScreen" :type-number="15" :select-number="selceNumber" @changeData="changeData" />
...
import { goodsAnchorState } from '@/utils/headerConfig.js'
export default {
  name: 'GoodsAnchor',
  components: { fliterArea, listScreen, BackToTop, Pagination, copyView, major },
  data() {
    return {
      listQuery: {
        current: 1,
  ...
  methods: {
    initTable() {
      if (JSON.parse(localStorage.getItem('goodsAnchorRank') === null)) {
        this.columnObj = goodsAnchorState
      } else {
        this.columnObj = JSON.parse(localStorage.getItem('goodsAnchorRank'))
      }
      var sum = 0
