<template>
  <div ref="myAppContainer" class="app-container">
    <div class="searchArea">
      <div class="page-title">我关注的小店</div>
      <div class="search-left">
        <div class="keyWordCss">
          <span class="pubilcSubtitle">关键字</span>
          <el-input v-model="listQuery.keyword" :disabled="isFreeMembership()" class="keyWordInput" clearable placeholder="请输入关键字搜索小店" />
        </div>
        <el-button class="searchBtn" type="primary" :disabled="isFreeMembership()" size="medium" @click="handleSearch">搜索</el-button>
      </div>
    </div>
    <listScreen ref="listScreen" :type-number="20" :select-number="selceNumber" @changeData="changeData" />
...
import { enjoyOrNotEnjoyShop, enjoyShopList } from '@/api/fav.js'
import { favShopState } from '@/utils/headerConfig.js'
export default {
  name: 'EnjoyShop',
  ...
  methods: {
    initTable() {
      if (JSON.parse(localStorage.getItem('favShopRank') === null)) {
        this.columnObj = favShopState
      } else {
        this.columnObj = JSON.parse(localStorage.getItem('favShopRank'))
      }
      var sum = 0
    ...
    getList() {
      ...
      enjoyShopList(this.listQuery).then(res => {
