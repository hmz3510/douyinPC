<template>
  <div ref="myAppContainer" class="app-container">
    <div class="page-title">我关注的主播</div>
    <fliterArea :use-number="1" :title-name="'主播类型'" :is-history="false" :is-anchor-data="true" @getData="getData" />
    <listScreen ref="listScreen" :type-number="17" :select-number="selceNumber" @changeData="changeData" />
    ...
    <div ref="tableDiv" class="tableArea">
      ...
      >
        ...
        <el-table-column key="20" min-width="90px" header-align="right" fixed="right">
          ...
          <template slot-scope="scope">
            <div class="btnList">
              <div class="listBTn" @click="handClick('anchorDetail', scope.row)">主播详情</div>
...
import { enjoyOrNotEnjoyAnthor, enjoyUserList } from '@/api/fav.js'
import { favAnchorState } from '@/utils/headerConfig.js'
export default {
  name: 'AnchorRank',
  ...
  methods: {
    initTable() {
      if (JSON.parse(localStorage.getItem('favAnchorRank') === null)) {
        this.columnObj = favAnchorState
      } else {
        this.columnObj = JSON.parse(localStorage.getItem('favAnchorRank'))
      }
      var sum = 0
    ...
    getList() {
      ...
      enjoyUserList(this.listQuery).then(res => {
