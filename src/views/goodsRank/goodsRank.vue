<template>
  <div ref="myAppContainer" class="app-container">
    <div class="page-title">热门商品榜</div>
    <prodScreen ref="prodScreen" @getData="getData" />
    <listScreen ref="listScreen" :type-number="4" :select-number="selceNumber" @changeData="changeData" />
    <div v-show="isScroll" class="headerTableCss" :style="{width:myHeaderWidth + 'px'}">
      ...
      >
        ...
        <el-table-column v-if="userInfomation.roleIdExplain === 'ADMIN'" key="14" width="124px">
          <template slot="header">
            <div class="hasLine">统计销量</div>
          </template>
        </el-table-column>
...
import { goodsState } from '@/utils/headerConfig.js'
export default {
  name: 'GoodsRank',
  ...
  methods: {
    initKeyWord() {
      if (this.$route.query.keyword !== undefined) {
        ...
        this.$refs.prodScreen.listQuery.keyword = this.$route.query.keyword
      }
      this.getList()
    },
    initTable() {
      if (JSON.parse(localStorage.getItem('goodsRank') === null)) {
        this.columnObj = goodsState
      } else {
        this.columnObj = JSON.parse(localStorage.getItem('goodsRank'))
      }
      var sum = 0
