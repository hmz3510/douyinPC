<template>
  <div ref="myAppContainer" class="app-container">
    <div class="page-title">历史直播数据库</div>
    <fliterArea ref="filterArea" :use-number="5" :title-name="'直播类型'" :is-history="true" :is-anchor-data="false" @getData="getData" />
    <listScreen ref="listScreen" :type-number="2" :select-number="selceNumber" @changeData="changeData" />
...
import { historyState } from '@/utils/headerConfig.js'
export default {
  name: 'LiveHistory',
  ...
  methods: {
    ...
    initTable() {
      if (JSON.parse(localStorage.getItem('historyRank') === null)) {
        this.historyState = historyState
      } else {
        this.historyState = JSON.parse(localStorage.getItem('historyRank'))
      }
      var sum = 0
    ...
    tips(data) {
      const routeData = this.$router.resolve({ path: 'big', query: { roomSn: data.roomSn }})
      window.open(routeData.href, '_blank')
    },
    restTable() {
      this.initTable()
