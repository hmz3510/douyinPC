<template>
  <div ref="myAppContainer" class="app-container">
    <div class="page-title">主播榜</div>
    <fliterArea :use-number="1" :title-name="'主播类型'" :is-history="false" :is-anchor-data="false" @getData="getData" />
    <listScreen ref="listScreen" :type-number="3" :select-number="selceNumber" @changeData="changeData" />
    <div v-show="isScroll" class="headerTableCss" :style="{width:myHeaderWidth + 'px'}">
      ...
      >
        ...
        <el-table-column v-if="columnObj.addFansExplain" key="4" label="涨粉数" width="124px">
          <template slot="header">
            <div class="headerCss" :style="{color:listQuery.sort === '1'? '#0E71EB': '#232333'}"><i :class="listQuery.sort === '1' ? 'el-icon-sort-down' : 'stand'" />涨粉数</div>
          </template>
        </el-table-column>
        <el-table-column v-if="columnObj.addFansRateExplain" key="5" label="涨粉率" width="124px">
          <template slot="header">
            <div class="headerCss" :style="{color:listQuery.sort === '3'? '#0E71EB': '#232333'}"><i :class="listQuery.sort === '3' ? 'el-icon-sort-down' : 'stand'" />涨粉率</div>
          </template>
        </el-table-column>
        <el-table-column v-if="columnObj.liveCount" key="6" label="开播场次" width="124px">
          <template slot="header">
            <div class="headerCss" :style="{color:listQuery.sort === '5'? '#0E71EB': '#232333'}"><i :class="listQuery.sort === '5' ? 'el-icon-sort-down' : 'stand'" />开播场次</div>
          </template>
        </el-table-column>
        <el-table-column v-if="columnObj.goodsLiveCount" label="带货场次" width="124px">
          <template slot="header">
            <div class="headerCss" :style="{color:listQuery.sort === '7'? '#0E71EB': '#232333'}"><i :class="listQuery.sort === '7' ? 'el-icon-sort-down' : 'stand'" />带货场次</div>
          </template>
        </el-table-column>
        <el-table-column v-if="columnObj.interateRateExplain" label="直播互动率" width="124px">
          <template slot="header">
        ...
        <el-table-column key="15" min-width="90px" header-align="right" fixed="right">
          <template slot="header">
    ...
    <div ref="tableDiv" class="tableArea">
      ...
      >
        ...
        <el-table-column v-if="columnObj.goodsLiveCount" key="7" label="带货场次" width="124px">
          <template slot="header">
            <div class="headerCss" :style="{color:listQuery.sort === '7'? '#0E71EB': '#232333'}"><i :class="listQuery.sort === '7' ? 'el-icon-sort-down' : 'stand'" />带货场次</div>
          </template>
          <template slot-scope="scope">
...
import { getUserRankList, enterUser } from '@/api/anchor.js'
...
import { anchorState } from '@/utils/headerConfig.js'
export default {
  name: 'AnchorRank',
  ...
  methods: {
    initTable() {
      if (JSON.parse(localStorage.getItem('anchorRank') === null)) {
        this.columnObj = anchorState
      } else {
        this.columnObj = JSON.parse(localStorage.getItem('anchorRank'))
      }
      var sum = 0
