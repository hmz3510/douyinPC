<template>
  <div>
    <div id="liveAddFans" class="chats" />
  </div>
</template>
...
export default {
  ...
  methods: {
    initChart() {
      this.chart = echarts.init(document.getElementById('liveAddFans'))
      this.chart.clear()
      var xArr = this.canvasData.dateKeyList
      var yArr = this.canvasData.followCountList
      this.chart.setOption({
        ...
        tooltip: {
          ...
          formatter: function(data) {
            ...
            var htmlStr = '<div>'
            htmlStr += '涨粉总数：' + data[0].data.label + '<br/>'
            htmlStr += date + '</div>'
            return htmlStr
          }
        },
