<template>
  <div>
    <div id="contribution" class="chats" />
  </div>
</template>
...
export default {
  ...
  methods: {
    initChart() {
      this.chart = echarts.init(document.getElementById('contribution'))
      this.chart.clear()
      var xArr = this.canvasData.goodsDateKeyList
      var yArr = this.canvasData.userContributionList
      ...
      this.chart.setOption({
        ...
        tooltip: {
          ...
          formatter: function(data) {
            ...
            htmlStr += '人均贡献：' + data[0].data.label + '<br/>'
            htmlStr += date + '</div>'
            return htmlStr
          }
        },
