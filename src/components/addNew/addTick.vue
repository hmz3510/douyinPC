  ...
  <div class="add-container">
    ...
    >
      <div class="diaLog-container">
        <div class="diaTitle">添加我的抖音号（{{ hasNumber }}/30）</div>
            ...
            <div class="listAreaContainer">
              <div v-for="item of list" :key="item.id" class="oneList">
                ...
                <div class="listRight">
                  ...
                  <div v-else :class="item.hasRecruit ? 'stopCss': 'addCss'" @click="handEnjoy(item)">{{ item.hasRecruit ? '取消': '添加' }}</div>
                </div>
              </div>
            </div>
          </div>
...
import { tiktokCancelOrAdd } from '@/api/tiktok.js'
...
export default {
  ...
  methods: {
    ...
    handEnjoy(data) {
      ...
      tiktokCancelOrAdd(userSn, url).then(res => {
        if (!data.hasRecruit) {
          this.$message({
            type: 'success',
            message: '添加成功!'
